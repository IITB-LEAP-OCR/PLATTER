...
from iou_utils.Evaluator import *
from iou_utils.utils import BBFormat
import sys
import os
    ...
    for f in files:
        ...
        for line in fh1:
            ...
            if isGT:
                ...
                x = float(splitLine[1])
                y = float(splitLine[2])
                w = float(splitLine[3]) - x
                h = float(splitLine[4]) - y
                ...
                x = float(splitLine[-4])
                y = float(splitLine[-3])
                w = float(splitLine[-2]) - x
                h = float(splitLine[-1]) - y
...
libPath = os.path.join(currentPath, 'iou_utils')
add_path(libPath)
...
if args.savePath is not None:
    ...
    savePath = os.path.join(currentPath, 'iou_utils/results')
# Validate savePath
